...
const Homepage = ({users, recipes, ingredients, handleIngredientSelected, updateUser, handleRecipeFavourite }) => {
  ...
  const foundRecipes = recipes.map((recipe) => {
    return <RecipeResult recipe = {recipe} key={recipe.id} handleRecipeFavourite={handleRecipeFavourite}/>
  })
